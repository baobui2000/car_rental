...
export default function Component() {
  ...
  const [rentalStatus, setRentalStatus] = useState<
    "idle" | "loading" | "success" | "passcode" | "guide" | "timer"
  >("idle");
  ...
  const handlePasscodeSubmit = () => {
    if (passcode.length === 4) {
      ...
      alert("パスコードは4桁である必要があります。");
    }
  };

  const guideSteps = [
    "タンダムかバギーのロックのところに鍵をタッチ",
    "設定したパスコードを入力してロックを解除します。",
    "利用して頂き、終わり次第アプリの画面上に「レンタル終了」押して頂けます。",
    "乗車を開始し、楽しんでください！",
  ];
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <Dialog open={rentalStatus === "passcode"}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>パスコードを設定</DialogTitle>
            <DialogDescription>
              次回の利用時に使用するパスコードを設定してください。
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
            <Input
          ...
          <DialogFooter>
            <Button onClick={handlePasscodeSubmit}>パスコードを設定</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
